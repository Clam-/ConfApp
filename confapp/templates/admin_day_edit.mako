...
<%
	...
	person = item.person
%>
	<form class="form-horizontal" action="${request.route_url("admin_day_edit", day=section.description, session=session.id, person=person.id)}" method="post">
...
%>
		...
		<div class="form-group">
			<label class="col-sm-2 control-label small" for="formSpecial">Special:</label>
			<div class="col-sm-5 ">
				<label class="checkbox-inline">
					<input type="hidden" name="special" value="_" />
					<input id="formSpecial" type="checkbox" name="special" value="True" />
				</label>
			</div>
		</div>
